...
2. 每次上传返回任务 ID；任务按 `queued → parsing → extracting → merging → persisting → awaiting_review → completed` 转换，任一非终态可转 `failed` 或 `cancelled`，全部可通过 SSE 观察。终态为 `completed`、`failed`、`cancelled`；转换触发者、取消竞争与重连语义归 A03。
