...
7. **抽取质量（赛题「技术要求与指标（一）」硬指标，REQ-01）**：
   ...
   - **准确率**：人工抽样中判定正确的比例不低于 70%，恰为 70% 视为达标。实体和关系分别计算，两项都须达标（赛题没有区分，按较严的理解执行；D-14，ArvinHan 2026-09-24 确认）。判定标准和抽样方法（固定随机种子，写明样本量或全量检查）写入 `evaluation/README.md`；报告逐条列出被抽样的项和判定结果。
