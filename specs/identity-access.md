...
- 契约中作用于调用者本人数据的请求 schema（进度、问答等）**不得**定义表示调用者的 `user_id` 字段；运行时客户端即使多传了这类字段，也只能被忽略，不得影响写入对象。
  > **覆盖声明（ADR-064 决定 1，2026-09-26，待 ArvinHan 裁决）**：「只能被忽略」不适用于**请求体中受闭合 schema 校验**的多余 `user_id`。
  > `PUT /progress` 的 `ProgressUpdate` 是闭合对象（`api.v1.yaml`），请求体中夹带 `user_id` 即 schema 违约：按契约中该操作的 422 说明与
  > `specs/learning-path.md` §5，整批返回 422 `VALIDATION_ERROR` 且零写入——不是忽略，也不是只按 A 的身份继续写。本条其余部分不变：
  > 调用者身份仍只取自令牌，写入的 `user_id` 一律取自调用者；**查询参数与自定义请求头**（`?user_id=`、`X-User-Id`）不受请求体 schema
  > 校验，仍按本条被忽略、不参与身份判定。冲突与裁决入口见 `docs/decisions.md` ADR-064 签收行；问答接口（J07，尚未实现）的 `ChatRequest`
  > 目前未声明 `additionalProperties: false`，落地时须先定这条并按同样方式登记。
...
| `getUploadPolicy` | `GET /api/v1/courses/{cid}/upload-policy` | 401 | COURSE | ROLE | ✓（ADR-022） |
...
- 失败路径
  - **IAM-14**（I02、C03）学生 A 在 `PUT /progress` 请求体中夹带 `user_id = B`，或加请求头 `X-User-Id: B` → 只改动 A 的进度，B 的进度逐字节不变。
    > **覆盖声明（ADR-064 决定 1，2026-09-26，待 ArvinHan 裁决）**：请求体夹带 `user_id` 的预期改为 422 `VALIDATION_ERROR`、整批零写入，
    > A 与 B 的进度都逐字节不变（依据见 §2.3 覆盖声明）；加 `X-User-Id` 请求头一条不变——该头不被读取，调用者仍是 A，只改动 A 的进度。
