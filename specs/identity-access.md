...
| 负责 | 改动 |
| --- | --- |
| B08 | `securitySchemes.bearerAuth` 的说明改为「`role` 仅作前端路由提示，服务端不据此授权」；所有继承 `bearerAuth` 的操作补 `401` 响应（现只有 `login`、`listCourses` 声明了 401）；新增 `eventTicket` 方案（`type: apiKey`、`in: query`、`name: ticket`） |
| B09 | `Course` 增加必填 `my_role`（`$ref: Role`）；`listCourses` 描述改为 §4.4；`login` 补 `429`；`getCourse` 的 `404` 说明加 `GRAPH_NOT_PUBLISHED`；新增 `listMembers`、`addMember`、`removeMember` 三个操作，`CourseMember`、`MemberAdd` 两个 schema，参数 `uid` |
