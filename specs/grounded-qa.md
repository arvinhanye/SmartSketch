...
J02 接受由已绑定 `PublishedVersion.graph_scope()` 生成的 `GraphScope`、可选知识点 ID 与检索术语。知识点 ID 仅在绑定副本中匹配时作为种子，否则忽略；术语对 `name` 与 `aliases` 作不区分大小写的包含匹配。按 ID 优先、其后 `kp_id` 字典序选取种子。最多扩展两跳，节点上限 32、结构关系上限 128；调用方可调低，不可调高。每跳只沿 `CONTAINS`、`PREREQUISITE`、`RELATED_TO`、`EXAMPLE_OF` 四类关系扩展，并将关系视作可双向遍历的邻接，输出保留原始方向。所有端点与关系必须同时匹配绑定的课程和 `version_id`。输出顺序稳定，未命中时返回空节点、空关系。图结构仅作无编号上下文，不是可引用证据。
