...
- **状态**：DRAFT。「问答终态与引用撤回协议」一节（Q1～Q12）由 A09 提交，**ADR-015 已签收**（ArvinHan，2026-09-23）；Q3.5 逐句覆盖与 Q10 日志覆盖范围按 **ADR-015 修订 1** 修订（ArvinHan，2026-09-24 签收）；相关度阈值、召回与 token 预算、性能门槛等仍是待细化项（见「待细化」）
...
**只有图证据的块进 A 的口径（ADR-065 决定 4，待签收）**：只来自图谱证据、没有相似度的块不参与闸门判定；一旦有任一向量候选达到阈值、闸门打开，它们可排到达标向量块之后并获得编号、进入 A（可被引用，J06 的三项复核不查阈值）。这是相对 Q1「从 H 中选出、达到阈值」字面的**扩大**，取舍为「偏向少答错、保留完整证据链」；K03 应把「引用来自图证据块」的答案单列统计。若签收时否决该扩大，则无相似度的块只作无编号上下文，J04 需相应改判。
...
- **「闸门已开但预算放不下任何块」的机器面区分（ADR-065 决定 5，待签收）**：该分支复用 wire 的 `below_similarity_threshold`（闭集所限），属语义合并；J10 必须另记 `over_budget` 与候选数、K03 按此分流，否则 QA-6/QA-7 与预算拒答在日志与评测口径上不可分。是否改为独立 `reason` 需 B13 同步契约与 `events.v1.md`。
