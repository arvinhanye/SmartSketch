...
- **I6** 草稿图谱只在 `persisting` 被写入；`failed` 或 `cancelled` 的任务不得在草稿中留下教师可见的节点或边（ADR-006 第 5 条）。取消只发生在 T5 之前，因此由取消引起的残留不可能出现；`persisting` 失败的清理见 §8.4。
...
| 阶段 | 接管后 | 幂等依据 |
| --- | --- | --- |
| `parsing` | 从阶段开头重跑 | 来源块 ID 由「文档 ID + 解析器版本 + 块序号」确定性生成，按 ID 覆盖写入，重跑不产生新块（D10） |
...
**`persisting`**（落实 I6，F13 实现）：

1. 取得课程写锁（§8.5），在**一个 Neo4j 事务**内用确定性 ID 以 `MERGE` 写入草稿（含 ADR-009 降级）。新节点 ID 由「课程 + 任务 + 候选键」确定；关系 ID 由「课程 + 类型 + 起点 + 终点」确定，两份资料产生的同一条关系合并为一条。本任务新建的节点、边与新增的来源关联标记 `created_by_task = task_id`（仅在创建时写入）。
2. Neo4j 提交后，在带令牌条件的 SQLite 事务内执行 T6 并分配 T6 提交序号（§3），然后释放课程写锁。
3. 在第 1、2 步之间崩溃：接管者重跑 `persisting`，`MERGE` 不产生重复，随后补做 T6。短暂窗口内草稿已可见而任务尚未到 `awaiting_review`；若任务最终仍 `failed`，由第 4 条清理。
4. `persisting` 以 `failed` 结束时：在课程写锁下撤销 `created_by_task` 等于本任务的节点、边与来源关联；被合并进来的已有节点保留，只撤销本任务加上的来源关联。清理失败（如 Neo4j 不可用）时任务照样 T9，并置 `cleanup_pending = true`，由回收步骤重试，成功后清除该标记。

**计费不重复**：每次模型调用写一条 `model_calls`，键为「任务 + 块（无块时为空）+ 用途 + 尝试序号」。真实花费照实记录；同一键的重放写入按键去重，统计只计一次。
...
| `failed` / `cancelled` 任务的来源块 | 与该任务的检查点同批删除 |
...
- 失败路径
  - **LEASE-12**（F13）`persisting` 最终失败的清理：本任务新建的节点、边与新增来源关联被撤销，被合并的已有节点保留；清理时 Neo4j 不可用 → 任务仍 `failed`、`cleanup_pending = true`，Neo4j 恢复后回收步骤完成清理并清除标记。
  ...
  - **LEASE-17**（E04、E12）同一 `model_calls` 记录重放写入两次 → 统计只计一次。
