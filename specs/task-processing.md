...
| 失败/取消后「再处理」的入口（重新上传或新增端点） | **已定（D-16）**：MVP 只靠重新上传（新资料、新任务）；再处理端点留作后续任务。`Document.parse_status` 读时取该资料最新创建任务的 `stage`，`Document.task_id` 为同一任务的 ID（ADR-021）。全部任务为 `failed`/`cancelled` 的旧资料可经 `deleteDocument` 删除（ADR-021） |
...
| `merging` | 从阶段开头重跑 | 重复裁决与定义归并（E10）也调用模型；E12 持有其缓存，身份至少包含「课程 + 有序候选对 + 两侧完整规范化模型输入摘要（名称、类型、原定义、来源 ID/定位/原文）+ 提示词用途/版本/正文摘要 + 模型版本」，任一输入或提示词变化即失效；重跑命中缓存不再计费。E10 只返回调用元数据，不读写缓存 |
| `persisting` | 整段重跑 | 见下 |

> **`extracting` 行由 E12 落实（ADR-023，2026-09-26）**：检查点表为 `task_chunk_checkpoints`（迁移 008），单元分块（实体）与小节（关系，E11）两种；小节 = 资料修订 + 章节标题路径，只含成功块；小节关系抽取失败只记检查点，不计入 §5 阈值。块最终失败码：输出不合规 `EXTRACTION_INCOMPLETE`、其他模型调用错误 `LLM_UNAVAILABLE`、预算被拒 `BUDGET_EXCEEDED`。
...
| `stage = failed` ⇔ `error` 非空（`if/then` 或按状态拆分，Codex S07-R09） | B10 已完成（按状态拆分为 `Task` 四个分支，REVIEW-B10-R01） |
