...
> **F11 落实（ADR-060，2026-09-26）**：队列按请求时的 V 从草稿实时计算。低置信度关系 = 状态 `low_confidence`、两端未被拒绝的可见关系；疑似重复 = E08 名称归一（含已存别名）得到的一对，`similarity` 与 `reason` 随行；孤立知识点 = 发布后在图中没有一条边的未拒绝节点。各栏排序固定，键集分页（`kind` + `cursor` + `limit`），`totals` 为完整条数。单项处理 `resolveReviewItem`：关系 `approve`/`reject` 改状态；疑似重复 `merge`（走 F10）或 `reject`（不是重复）；孤立 `approve`（确认保留）或 `reject`（节点置 `rejected`）。已不在队列 404，同一动作重复提交 200 `changed = false`。
...
> **F12 落实（ADR-061，2026-09-26）**：教师的新建、修改、解锁、删除、合并各记一行 SQLite `graph_edit_logs`：操作者、时间、写入后的 `draft_revision`、节点修订号前后值、白名单且脱敏的变更摘要（合并含直接被合并节点，即直接父子关系）。`draft_revision + 1` 与 `pending` 行同一事务写入；Neo4j 返回后置 `committed`/`aborted`，失败可重试，遗留 `pending` 由下一次同课程教师写入持锁对账。只记真实写入，发布与回滚不写本日志（ADR-034）。
...
6. 审核队列能列出低置信度关系、疑似重复、孤立节点三类，且通过/拒绝/合并后队列相应更新。（F11，`tests/backend/test_f11.py`）
...
- ~~**审核队列的排序与分页**~~：各栏固定排序 + 键集分页（ADR-060）。
