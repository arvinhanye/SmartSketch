...
| `src/backend/app/repositories/graph_edit.py` | `DraftNodeStore`（读节点、条件更新/解锁、新建人工节点与来源、章节可见性）；SQLite：加草稿修订号、可用来源块 |
...
| 集成全量 | 同上，`tests/integration -q` | 123 passed, 4 skipped；合入 main 后 142 passed, 4 skipped |
| 后端全量 | `$S/pt.sh tests/backend -q` | 3021 passed；合入 main 后 3024 passed |
| 前端 | `npm --prefix src/frontend run type-check`；`npm --prefix src/frontend run test -- --run` | 通过；263 passed；合入 main 后 285 passed |
