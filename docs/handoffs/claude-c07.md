...
- `src/backend/app/api/materials.py`（新增）：`GET` / `POST /api/v1/courses/{cid}/documents`，`operation_id` 分别为 `listDocuments`、`uploadDocument`。只做协议转换：授权用 C03 的 `course_teacher`；上传经 `UploadFile`（字段 `file`）交给服务层；`FileStorageError` 映射为 `Error` 响应：`UNSUPPORTED_FORMAT` 415、`FILE_TOO_LARGE` 413（`details.limit_bytes`）、`VALIDATION_ERROR` 422、`STORAGE_UNAVAILABLE` 503。C05 文件名错误的 `details.fields` 在此转成全局校验同形 `[{"in": "body", "field": "file", "reason": …}]`，不回显文件名。
...
- `tests/backend/test_c07.py`（新增，39 项）。
- 范围扩展：
...
- FastAPI 在执行依赖前先解析整个 multipart 请求体（Starlette 把文件部分溢写到系统临时目录），所以匿名或越权的大请求在返回 401/403 前已被完整接收；`UPLOAD_MAX_BYTES` 只限制落到 `STORAGE_DIR` 的字节。生产部署需在反向代理设请求体上限（建议略大于 50 MiB），见待决 3。
...
3. **请求体上限前置**：是否在应用层按 `Content-Length` 提前拒绝，或只靠反向代理，需要决定（见风险 1）。
