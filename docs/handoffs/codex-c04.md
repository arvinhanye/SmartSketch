...
- 集成基线复验：`python -m pytest tests/backend/test_c04.py tests/backend/test_c03.py -q --tb=short` → 29 passed / exit 0；`python -m pytest tests/backend -q --tb=short` → 1060 passed / exit 0；`scripts/gen-contracts.sh --check` → exit 0；`scripts/verify.sh` 在 `PYTHONIOENCODING=utf-8` 下 → exit 0（契约负例 25 项通过）。B14 生成器回归 3 passed，伴随 6 条本机 GBK 子进程读取警告。PR CI 和合并后的 main 验证仍待执行。
