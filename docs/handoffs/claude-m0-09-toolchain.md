...
- **状态**：工具已安装并实测通过；两个脚本缺陷已修（`740adb` 的 `8865686`），但该修复在 UTF-8 locale 下引入了新的假绿（Codex S07-R06），已在 `978671e` 修复；**生成物入库未做**（需在选定基线上做）
...
- **跨 worktree 改动**：经用户授权，在 `claude/worktree-contract-conflicts-740adb` 上提交了 `8865686`（只改 `scripts/gen-contracts.sh`），以及修复其 UTF-8 假绿回归的 `978671e`（改 `scripts/gen-contracts.sh`、新增 `tests/contracts/test_contracts.py` 的一条回归测试）。除此之外所有实测都在 scratch 副本里做（`git archive` 出 `scripts/` 与 `src/contracts/`）
...
## 三、实测暴露的两个缺陷（已修，`740adb` 的 `8865686`；其引入的 R06 回归见本节末更正）
...
| `./scripts/verify.sh` | `exit 1` | `exit 1`（原因变了，见第四节）——**仅 C locale；UTF-8 下为 exit 0 假绿，见本节末更正** |

### 完整回归矩阵（scratch 副本，13 项；**只在 C locale 下跑过**，见本节末更正）
...
### 更正：`8865686` 在 UTF-8 locale 下引入的假绿（Codex S07-R06，已在 `978671e` 修复）

上面「修复前后对照」和 13 项矩阵都是在**未设置 LANG 的 shell（即 C locale）**里跑的。Codex 审查在 UTF-8 locale 下复现出：缺产物时 `--check` 打印 `name�: unbound variable` 却 **exit 0**，`verify.sh` 输出 "All verification checks passed"。我复现确认：

```text
8865686  LC_ALL=C            exit 1（我当时的环境）
8865686  LC_ALL=en_US.UTF-8  exit 0  假绿
8865686  LC_ALL=zh_CN.UTF-8  exit 0  假绿
```

根因两层：(1) 我写的 `"$OUT/$name（…"` 未加花括号，bash 3.2（`env bash` 解析到的就是 macOS 自带 3.2.57）在 UTF-8 locale 下把「（」的字节并入变量名，`set -u` 中止；(2) bash 3.2 被 `set -u` 中止后进入 EXIT trap 时 `$?` 已是 0——实测 trap 首行 `rc=$?` 就拿到 0，所以「trap 里保存 `$?` 再原样退出」在这个版本下救不回来。

`978671e` 的修复与验证：

- 该行改为 `"${OUT}/${name}（…"`；全部 12 个 shell 脚本按字节扫描，只有这一处。
- EXIT trap 加完成守卫：脚本末尾置 `_completed=1`，trap 里 rc 为 0 却没走到结尾就按失败处理。单独验证：只撤掉花括号、保留守卫，UTF-8 下 exit 1 并打印「未正常结束」——两道防线互相独立。
- 回归测试 `test_gencheck_missing_stage_fails_in_every_locale`（先写、先红、后修）：覆盖 C 与全部可用 UTF-8 locale；修复前 `[en_US.UTF-8] 缺 openapi.json` 实际 exit 0，修复后通过；没有 UTF-8 locale 时直接失败，不静默跳过。
- 真实分支 `--check`：C / en_US.UTF-8 / zh_CN.UTF-8 均 exit 1；契约测试 21/21；`verify.sh` 在 C 与 en_US.UTF-8 下均 exit 1「验证失败：contracts」。

教训：门禁类脚本的「必须失败」用例要在至少一个 UTF-8 locale 下跑，C locale 下的绿不能代表用户终端里的结果。
...
**该分支的 `./scripts/verify.sh` 现在是 `exit 1`，这是正确结果，不是脚本缺陷**（更正：`8865686` 时这句只在 C locale 下成立，UTF-8 下是 exit 0 假绿；`978671e` 后 C 与 UTF-8 均实测 exit 1）：`scripts/verify/contracts.sh` 调的是 `gen-contracts.sh --check --allow-scaffold`，而本机生成器已齐全，`--allow-scaffold` 不再跳过任何阶段，于是 `--check` 如实报出 `python/` 与 `typescript/` 没有入库。装生成器之前它是绿的——那个绿是降级模式给的。
...
**Backend Agent，M0-09**：脚本缺陷已修（`740adb` 的 `8865686`，其 UTF-8 假绿回归已在 `978671e` 修复），剩下的是 (1) 在选定的集成基线上跑 `./scripts/gen-contracts.sh` 把 `python/` 与 `typescript/` 补齐入库，(2) 删掉 `scripts/verify/contracts.sh` 里的 `--allow-scaffold`，让缺工具直接红掉，(3) 顺手补上第三节末尾那个「顶层陈旧阶段检不出」的反向遍历。
...
git -C .claude/worktrees/worktree-contract-conflicts-740adb revert --no-edit 978671e   # 先撤 R06 修复
git -C .claude/worktrees/worktree-contract-conflicts-740adb revert --no-edit 8865686   # 再撤脚本修复
```
