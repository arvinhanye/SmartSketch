...
| `tests/frontend/h11.test.ts`（文件锁） | 45 项：卡片视图模型排序/不改输入；API 必带 `version`、非法版本不发请求；不取草稿 8 项（未发布、`GRAPH_NOT_PUBLISHED`、课程内教师、版本号透传、4 种异常响应、卡片不另发请求、课程页入口、路由放行）；状态 6 项（加载、空图、网络重试、版本已替换、无权限、切课迟到响应）；图/卡片联动 5 项；分页 8 项；键盘 7 项 |
