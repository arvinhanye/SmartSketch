...
未改：契约、后端、`main.ts`、路由与页面。面板**未**接入页面（留 H11）。
...
1. ADR-062 签收。
2. 教师图谱页中编辑面板与 H06 详情抽屉如何切换（H11）。

## 下一步

- H11：`<NodeEditor :kp-id="selected" @saved="…" @deleted="selected = null" @refresh-needed="重新加载草稿图谱" @close="selected = null" @course-forbidden="回课程列表" />`；如需统一注入，在 `main.ts` provide `NODE_EDIT_API_KEY: createNodeEditApi(http)`。
