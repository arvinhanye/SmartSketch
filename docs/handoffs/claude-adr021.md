...
- 审查发现：上传后触发的静默列表刷新若在删除成功后才返回，会把已删除的行带回来（再点删除得 404 后才消失）。
- 修复：`useMaterials` 的页面会话记录本页已删除的资料 ID，列表响应按它过滤。先加复现测试（1 failed），修复后 `h02.test.ts` 80 passed，前端全量 263 passed，`type-check` exit 0。
- 未改：`reload()` 与在途加载重叠的问题在界面上不可达（重试按钮只在错误态显示，加载开始即离开错误态），不改。
