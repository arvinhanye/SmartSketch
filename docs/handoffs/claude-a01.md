...
- **合入注意**：main 的 `docs/architecture.md` 与 `docs/tasks.md` 当前带有未提交的 Codex PLAN-01 改动（architecture 顶部新增入口段；tasks 新增 PLAN-01 与「待确认决策」节，且把「未决问题」改名）。本 worktree 基于 `05d214c`，两处改动位置不重叠，但**必须按 diff 叠加，不要整文件覆盖**。`docs/decisions.md` 在 main 未被改动，可直接叠加。
