...
- C01 的 SQLite 连接从已校验的 `SQLITE_URL` 取路径，启用 WAL、外键，并将每个连接的 `busy_timeout` 固定为 5000 ms。迁移版本记录在 `schema_migrations(version, filename, checksum, applied_at)`；`001_base.sql` 用 `CREATE TABLE IF NOT EXISTS` 接管 B06 的 `embedding_space_state`，不覆盖现存单行。迁移只向前，逐文件在写事务内应用并记录摘要；已应用文件摘要变化即拒绝。每次待执行迁移前，按 `specs/task-processing.md` §8.7 检查有效租约/课程锁、`VACUUM INTO` 备份并验证完整性；恢复须停机并替换数据库文件。
