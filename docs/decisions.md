...
- **决定**（接续决定 1～4）：
  ...
  9. **显式写入覆盖继承**（细则 1，修订决定 2 的“取最高”）：设来源 `m` 在绑定版本 V（版本号 `n`）中按 A08 §5 的链规则归属到可见节点 `p`。
     ...
     **补注（Codex A08S-R01，推导细则，待 ArvinHan 签收）**：同值写入不能只凭原始 `status` 相同就跳过。在提交时的最终绑定版本上，若 `p` 仍有未被覆盖的来源，同值写入也是一次显式写入，取新写入序号并覆盖它们；没有时才为无操作（不取序号、不改 `updated_at`）。因此同一绑定版本下的重放从第二次起为无操作。见 A08 §5「同值写入」与 LP-16、LP-18。
