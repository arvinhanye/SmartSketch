...
- **决定**：
  ...
  5. **幂等**：`parsing` 按确定性块 ID 覆盖写；`extracting` 按块检查点续跑；`merging` 重跑、模型结果靠 E 组的缓存；`persisting` 在课程写锁下以单个 Neo4j 事务 `MERGE` 写入、再以带令牌条件的 SQLite 事务执行 T6；最终失败按 `created_by_task` 撤销本任务新建的内容（**修订 1 改为按任务贡献撤销，失败即不可见**），清理失败置 `cleanup_pending` 由回收重试。`model_calls` 按「任务 + 块 + 用途 + 尝试序号」去重统计。
