...
- **计量**：只计 LLM 调用，数值为供应商返回 usage 中输入与输出 token 之和；重试、备用调用、E05 修复调用都计入。向量调用只记入 `model_calls`，不计入预算（单价低，总量受资料篇幅约束）。fake 模式按确定规则上报模拟 usage，使预算逻辑可测。
- **软上限**：发起每次调用前检查「已用 ≥ 上限」，成立则不发；在途调用照常完成并计入，因此实际用量至多超出「并发数 × 单次调用 token」。`0` 表示不发任何请求（E04 验收）。不提供「不限」写法，需要放开时写一个足够大的数，避免漏配时悄悄无上限。
- **任务预算**：按任务累计，覆盖该任务的全部尝试（ADR-011 的 L3 接管后继续累计，不清零），数据来自 SQLite `model_calls`，去重规则沿用 ADR-011（任务 + 块 + 用途 + 尝试序号）。
- **每日预算**：按北京时间（UTC+8）自然日汇总 `model_calls`，时间用 SQLite 求值（与 ADR-011 一致），因此多个进程共享同一额度。
