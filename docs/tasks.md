...
- G03 验收：物化只写 `(course_id, 新 version_id)`，旧版本副本与发布指针不变，学生照读旧版；向量空间或维度不符、缺向量在连库前失败，缺来源块整体回滚；重试同一版本先删后建、不重复；P9 读回复算摘要一致。G03 待决：已发布知识点对外状态恒为 `approved`、来源类别恒为 `manual`（ADR-033 第 5 条，待确认）；已发布详情的来源没有原文片段。
