...
| 原子 ID | 状态 | 任务 | 负责人 | 分支 / base | 文件锁（本轮唯一写入者） | 证据 |
| --- | --- | --- | --- | --- | --- | --- |
| B12-R1 | DONE（待 PR 审查/合并） | 进度契约错误细节修订（ADR-017 决定 4、5） | ArvinHan（Claude 子代理） | `claude/b12-r1-progress-errors` / ADR-017 提交 | `src/contracts/api.v1.yaml`、`src/contracts/errors.v1.md`、`src/contracts/v1/generated/`、`tests/contracts/test_b12.py`、`specs/learning-path.md`、`docs/handoffs/claude-b12-r1.md` | 改真源前 B12 34 failed / 88 passed，改后 122 passed；契约全量 288 passed；`./scripts/gen-contracts.sh --check` exit 0；`./scripts/verify.sh` exit 0（含 B12 122 项）；生成 TS `tsc --noEmit --strict` exit 0；`git diff --check` exit 0；反向篡改 4 处均被检出；`tests/contracts tests/tooling` 1 failed / 301 passed，失败项 `test_b07.py` 夹具缺 `test_b14.py`，基线 `d446ecc` 同样失败，超出文件锁未修；待决：`field` 取 `[<i>].kp_id`（ADR 原文）还是 C13 点路径；`docs/handoffs/claude-b12-r1.md` |
