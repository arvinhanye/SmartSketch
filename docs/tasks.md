...
| D-09 | 前端登录页与会话存储缺少原子任务：A05 §2.4 要求令牌与 `LoginResponse.user` 存 `sessionStorage`、401 清令牌跳登录页，但 G-1 只列 C13～C16（后端）与 H12（成员页）。B03 目前以注入 `getAccountRole` 取得账号类型，应用入口恒为未登录。需决定是否新增前端登录任务（登录页、会话读写、401 处理）及编号 | 产品负责人 / 协调 Agent | H01 开始前 |
...
- 交出的后续项（均未认领）：B15 须把 `scope.signal` 传给 fetch，旧请求才会在网络层真正取消；路由 `cid` 与 `selectCourse` 的接线归 H01；问答「最新回答与引用」槽位由 J08/J09 决定是否加入并在切课时清空。
