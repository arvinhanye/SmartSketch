...
| 原子 ID | 状态 | 任务 | 负责人 | 分支 / base | 文件锁（本轮唯一写入者） | 证据 |
| --- | --- | --- | --- | --- | --- | --- |
| E09 | DONE（待 PR 审查/合并） | 实现向量候选分层 | ArvinHan（Claude 子代理） | `claude/e09-vector-tiers` / 本认领提交 | `src/backend/app/services/fusion/candidates.py`、`tests/backend/test_e09.py`、`docs/handoffs/claude-e09.md` | `test_e09.py` 84 passed（红：`ModuleNotFoundError`）；后端全量 2605 passed；边界：`≥ auto_merge` 自动、`review ≤ s < auto_merge` 裁决、`< review` 保留；阈值须有限、`[0, 1]`、`review < auto_merge`，无默认值（D-08 未签收）；跨课程/跨向量空间混传整体拒绝（`VectorIsolationError`）；11 处反向篡改均检出；`./scripts/verify.sh` exit 0、`git diff --check` exit 0；待决 3 项见 `docs/handoffs/claude-e09.md` |
