...
| D-02 | 首个 OpenAI 兼容模型供应商与预算上限。A07 已拆为 D-02a～f 六项，签收入口见 `docs/integrations.md`「待签收取值（D-02）」；配置形状与规则已定。**D-02a 已签收**：主用 DeepSeek V4.1 Flash（`deepseek-flash`，ADR-024，ArvinHan 2026-09-26）；D-02b～e 仍未签收 | 技术负责人 | 接入抽取服务前（fake 实现可先行） |
...
- 待决：
  ...
  2. **D-02**：D-02a 已签收（ADR-024，DeepSeek V4.1 Flash）。还需 D-02d 预算上限与付费调用确认，之后按 `evaluation/reports/extraction-accuracy.md` 第 4 节命令跑真实模型并人工判定；完整流程（融合去重后的草稿）另需 E12。
