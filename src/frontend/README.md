...
B01 建立了 Vue 3 + TypeScript + Vite 单页应用，B02 建立了 Vitest 测试配置，B03 加入教师/学生路由壳（`src/router/`、`src/views/`），B04 加入 Pinia 课程上下文（`src/stores/course.ts`）。业务页面由后续任务加入；登录页与会话存储由 H13 接入，在此之前应用入口恒为未登录。
